...
// TestUpstreamAuthJWTHarness is the vendored upstream test, renamed and skipped.
//
// It fails with "401 expected, got 200" against the Pantacor go-json-rest fork,
// and it fails IDENTICALLY when run against the pristine upstream module -- so
// this is inherited breakage, not a vendoring regression. Driving the same
// middleware through net/http/httptest shows it behaves correctly (see
// contract_test.go), which puts the fault in go-json-rest's rest/test harness
// rather than in the middleware.
//
// Kept rather than deleted so the upstream coverage is visible to whoever ports
// this to echo: the real assertions now live in contract_test.go.
func TestUpstreamAuthJWTHarness(t *testing.T) {
	t.Skip("rest/test harness is broken against the Pantacor fork; see contract_test.go")
