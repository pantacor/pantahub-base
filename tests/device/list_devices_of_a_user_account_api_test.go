...
func TestListDevicesOfUserAccount(t *testing.T) {
	...
	if res.StatusCode() != 200 {
		t.Errorf("%s", "Error Creating Device1:Expected Response code:200 but got:"+strconv.Itoa(res.StatusCode()))
		t.Error(res)
	}
	device2, res := helpers.CreateDevice(t, true, "345")
	if res.StatusCode() != 200 {
		t.Errorf("%s", "Error Creating Device2:Expected Response code:200 but got:"+strconv.Itoa(res.StatusCode()))
		t.Error(res)
	}
	device3, res := helpers.CreateDevice(t, true, "678")
	if res.StatusCode() != 200 {
		t.Errorf("%s", "Error Creating Device3:Expected Response code:200 but got:"+strconv.Itoa(res.StatusCode()))
		t.Error(res)
	}
